...
function wp_phpdoc_markdown_append_hook_doc( $hook, $file, $source_dir ) {
	...
	$template = str_replace( '{{SUMMARY}}', $hook['doc']['description'], $template );
	$template = str_replace( '{{DESCRIPTION}}', $hook['doc']['long_description'], $template );
	...
	if ( count( $arguments ) > 0 ) {
		foreach ( $arguments as $argument ) {
			...
			if ( isset( $params_doc[ $argument ] ) ) {
				...
				$parameters = str_replace( '{{DESC}}', $params_doc[ $argument ]['description'], $parameters );
			} else {
...
function wp_phpdoc_markdown_get_template_build_changelogs( $hook ) {
	...
	foreach ( $sinces as $since ) {
		...
		$version      = str_replace( '{{DESC}}', $since['description'], $version );
